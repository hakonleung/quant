...
from quant_core.config import get_settings, init_settings
...
def _data_root() -> Path:
    ...
    Sourced from :mod:`quant_core.config` (``QUANT_DATA_ROOT``).
    Defaults to ``./data`` relative to the **process CWD** when unset —
    brittle, but the fail-fast assertion in :func:`_assert_data_root_ok`
    catches mis-pointed roots on startup.
    """
...
def _bootstrap_settings() -> None:
    """Parse the env keys this service cares about and seed the ConfigCenter.

    Keep this list aligned with ``.env.example`` — never expand without a
    matching field on :class:`quant_core.config.Settings`.
    """
    values: dict[str, object] = {}
    if (v := os.environ.get("QUANT_DATA_ROOT")) is not None and v != "":
        values["data_root"] = v
    if (v := os.environ.get("QUANT_LOG_LEVEL")) is not None and v != "":
        values["log_level"] = v
    if (v := os.environ.get("QUANT_FLIGHT_HOST")) is not None and v != "":
        values["flight_host"] = v
    if (v := os.environ.get("QUANT_FLIGHT_PORT")) is not None and v != "":
        values["flight_port"] = int(v)
    if (v := os.environ.get("US_WATCH_SOURCE")) is not None and v != "":
        values["us_watch_source"] = v
...
def main() -> int:
    ...
    parser.add_argument("--host", default=settings.flight_host)
    parser.add_argument("--port", type=int, default=settings.flight_port)
    ...
    us_backend_choice = settings.us_watch_source.strip().lower()
