...
Pulls qfq close series for the candidate universe in one
:meth:`KlineRepo.get_universe_slice` call, then for each code slides a
``window_days``-length window over its bars (one trading-day step) and
scores each window against the z-scored reference using DTW with a
Sakoe-Chiba band.

Cheap pre-filter: candidate windows whose first/last close ratio differs
from the reference's ratio by more than a configurable factor are
skipped before the DTW step (doc §4 "v1 优化"). Tuned conservatively
(factor 3.0) so it never drops a true match — only the obvious
non-fits.
...
class DTWPatternEngine:
    ...
    def find_similar(self, query: PatternQuery) -> list[PatternMatch]:
        ...
        # Pull the slice once for the whole universe.
        end = query.asof_end
        start = _earliest_window_start(end, query.window_days, query.lookback_days)
    ...
    def _scan_one_code(
        ...
        for i in range(len(clean) - window_days + 1):
            ...
            cand_ratio = _safe_ratio(float(closes[-1]), float(closes[0]))
...
def _validate_query(query: PatternQuery) -> None:
    ...
    if query.lookback_days < query.window_days:
        raise QuantError(
            "INVALID_ARGUMENT",
            f"lookback_days ({query.lookback_days}) must be >= window_days ({query.window_days})",
        )
    if query.top_n <= 0:
        raise QuantError("INVALID_ARGUMENT", f"top_n must be > 0, got {query.top_n}")
    src = query.reference.source
    if src.kind == "from_stock":
        # Reference must end strictly before the scan start to avoid
        # leaking future bars into the candidate windows.
        scan_start = _earliest_window_start(query.asof_end, query.window_days, query.lookback_days)
        if src.end_date >= scan_start:
            raise QuantError(
                "PATTERN_REFERENCE_LOOKAHEAD",
                f"reference end_date {src.end_date} overlaps the scan range "
                f"[{scan_start}, {query.asof_end}]",
                {"reference_end": src.end_date.isoformat()},
            )


def _earliest_window_start(end: date_cls, window_days: int, lookback_days: int) -> date_cls:
    from datetime import timedelta

    return end - timedelta(days=lookback_days + window_days)
