/**
 * BFF: DELETE /api/watch/:market/:code → NestJS `/api/watch/:market/:code`.
 *
 * Direct passthrough — NestJS replies 204 No Content; we mirror that
 * status so react-query treats the mutation as success.
 */
...
interface RouteCtx {
  readonly params: Promise<{ readonly market: string; readonly code: string }>;
}
...
export async function DELETE(request: Request, ctx: RouteCtx): Promise<Response> {
  ...
  const { market, code } = await ctx.params;
  const url = `${base}/api/watch/${encodeURIComponent(market)}/${encodeURIComponent(code)}`;
