...
describe('GET /api/stocks/* (HTTP contract)', () => {
  ...
  it('GET /api/stocks/:code returns a valid StockMetaDto', async () => {
    const res = await request(app.getHttpServer()).get('/api/stocks/600519').expect(200);
    expect(() => StockMetaDtoSchema.parse(res.body)).not.toThrow();
    expect(res.body).toMatchObject({ code: '600519', name_pinyin: 'GZMT' });
  ...
  it('GET /api/stocks/:code → 404 with STOCK_NOT_FOUND envelope when missing', async () => {
    const res = await request(app.getHttpServer()).get('/api/stocks/999999').expect(404);
  ...
  it('GET /api/stocks/batch?codes=a,b returns rows in input order', async () => {
    const res = await request(app.getHttpServer())
      .get('/api/stocks/batch?codes=600519,000858')
      .expect(200);
    expect(res.body.map((r: { code: string }) => r.code)).toEqual(['600519', '000858']);
  ...
  it('GET /api/stocks/batch silently skips unknown codes', async () => {
    const res = await request(app.getHttpServer())
      .get('/api/stocks/batch?codes=600519,does-not-exist,000858')
      .expect(200);
    expect(res.body.map((r: { code: string }) => r.code)).toEqual(['600519', '000858']);
  });
  ...
  it('GET /api/stocks/by-industry returns sorted results', async () => {
    const res = await request(app.getHttpServer())
      ...
      .query({ sw_l2: '白酒' })
      .expect(200);
    expect(res.body.map((r: { code: string }) => r.code)).toEqual(['000858', '600519']);
  });
