/**
 * Pure derivations from a list of K-line bars. Single source of truth
 * shared by:
 *   - the FE list panel (`feat-eq-list`) — used to derive price /
 *     chgPct / turnoverRate / turnover / consecUp from a 250-bar fetch.
 *   - the NestJS `StockListService.assembleRows` — same math when the
 *     applied column set asks for those fields.
 *
 * All inputs are immutable; functions are total (no throws).
 */

import type { KlineBar } from '../types/eqty.js';
...
export function deriveStockStats(bars: readonly KlineBar[]): StockStats {
