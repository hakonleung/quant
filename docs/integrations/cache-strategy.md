...
| 模块               | 路径                                  | 粒度                                                     | TTL / 失效                                                      |
| ------------------ | ------------------------------------- | -------------------------------------------------------- | --------------------------------------------------------------- |
| stock-meta         | `data/meta/stocks.parquet`            | 单文件全宇宙                                             | 手动 / BJT 16:30 cron 替换                                      |
...
| blacklist          | `data/blacklist.json`                 | 单文件 `{codes, asof, universeSize, computedAt}`         | 每个 16:30 BJT 批次的 BatchSettler 收尾重算 (`12-blacklist.md`) |
...
字段新增 → 直接改 schema 写新版本 → 启动时检测旧版本数据，单文件触发重建。v1 数据可重建，无需双写或额外脚本。
