...
| 模块 | 路径 | 粒度 | TTL / 失效 |
| ---- | ---- | ---- | ---------- |
| stock-meta | `data/meta/stocks.parquet` | 单文件全宇宙 | 手动 / cron 替换 |
| kline | `data/kline/<code>.parquet` | 一股一文件 | 增量（按水位） |
| sentiment | `data/sentiment/<kind>/*.parquet` | code 或 codes_hash | 2 交易日 |
| watch quotes | `data/watch/quotes/<code>.json` | 一股一 KV | 盘中刷新覆盖 |
| watch tasks | `data/watch/tasks.json` | 整文件 | 进程内为准 |
| notify dedupe | `data/notify/<sha>.json` | 一事件一 KV | 1 小时 |
