...
| 沪市         | SH                    | 60 / 688 / 689 / 900 开头                            |
| 深市         | SZ                    | 000 / 001 / 002 / 003 / 300 / 301 开头               |
| 北交所       | BJ                    | 4 / 83 / 87 / 88 / 920 开头（920 为 2024 年新增前缀） |
| 主板         | MainBoard             |                                                      |
| 创业板       | ChiNext               | 300 开头，深市                                       |
| 科创板       | STAR                  | 688 开头，沪市                                       |
| 北交所板块   | BSE                   | 4 / 8 / 920 开头                                     |
...
| 术语              | 含义                                                      |
| ----------------- | --------------------------------------------------------- |
| AKShare           | v1 主行情 / 新闻 / 研报源（含其代理的 sina / eastmoney）   |
| Baostock          | 备用日线源（v2 视情况引入）                                |
| 同花顺 / 东方财富 | 二源候选（爬虫，待引入；详见 `docs/todo-enhancement.md`） |
...
- 所有 **股票代码** 标准格式：**裸 6 位字符串**，如 `600519` / `000001` / `430047`。SH / SZ / BJ 三地 6 位代码空间不重叠，无需后缀消歧；需要交易所信息时由消费方按前缀派生（前缀映射见 `docs/modules/01-stock-meta.md` §3.1）
- 所有 **金额** 单位：元；**成交量** 单位：股
