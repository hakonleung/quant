...
- **替换** 旧 `push` / `SlackWebhookWatchNotifier` 单通道实现，统一为多 IM (slack / feishu，可扩展) 的 outbound + inbound 模块。
...
- 旧 `apps/api/src/modules/push/` 已删除；旧 `WatchNotifier` / `SlackWebhookWatchNotifier` 已删除。watch hit 触发路径走 `ChannelService.broadcast`。
- 旧 `POST /api/push/test` 由 `POST /api/channel/send` 替代；前端如有引用需要切换（M-checklist 已列出）。
- 共享 schema 在 `packages/shared/src/types/channel.ts` 与 `packages/shared/src/types/socket.ts`，TS 双侧用同一份 zod。Python 侧的兜底 `quant_io/notify` 暂未触达，跨进程契约后续再合并。
