...
| Settler      | `apps/api/src/modules/orchestration/batch-settler.ts`          | 监听 meta + kline 队列的终止事件（含失败到上限），按 `batchId` 收尾 → blacklist / DDE / metrics / dynamic sectors                                     |
...
flowchart TD
  ...
  FF --> Bf[stock-metrics-backfill<br/>(WCMI 横截面打分)]
  Bf --> Sec[dynamic sectors 全量重算]
```
...
- **Coalesce**：统一 scan 的 in-flight Promise 覆盖巡检、队列消费和收尾全过程，避免更新未完成时重复触发相同全市场任务。
