...
| Source   | `quant_io/sources/{akshare_watch,yfinance_watch,routed_watch}.py`                     | A/HK 走 `AKShareWatchSource`（`stock_bid_ask_em` / `stock_hk_hist_min_em`）；US 默认走 `YFinanceWatchSource`（`yfinance.Ticker.history(period=1d, interval=1m)`,Yahoo IP 段,避开东财/新浪封锁）,设 `US_WATCH_SOURCE=akshare` 可切回 `stock_us_hist_min_em`;`MarketRoutedWatchSource` 装配层按 market 派发。Universe 刷新（`stock_{hk,us}_spot_em`）始终用 AKShare |
