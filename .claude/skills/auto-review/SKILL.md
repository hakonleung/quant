...
You enforce `CLAUDE.md` end-to-end on the current change set. Run all gates. Do not stop on the first failure — collect everything and report once.
...
> Review the current change set against CLAUDE.md and docs/. Files: <list from step 1, classified by layer/process>. Apply the multi-language checklist. Return verdict per the format in your agent definition.
